...
class WaveNetModel(object):
    ...
    def _generator_conv(self, input_batch, state_batch, weights):
        ...
        output = tf.matmul(state_batch, past_weights, name="TEST_base1") + tf.matmul(
            input_batch, curr_weights, name="TEST_base2")
        return output
    ...
    def _generator_dilation_layer(self, input_batch, state_batch, layer_index,
        ...
        if local_condition_batch is not None:
            output_filter += tf.matmul(local_condition_batch, variables['cond_filter'][0, :, :], name="TEST_lc1")
            output_gate += tf.matmul(local_condition_batch, variables['cond_gate'][0, :, :], name="TEST_lc2")        
            ...
            for layer_index, dilation in enumerate(self.dilations):
                with tf.name_scope('layer{}'.format(layer_index)):
                    ...
                    output, current_layer = self._generator_dilation_layer(
                        current_layer, current_state, layer_index, dilation, local_condition_batch,
    ...
    def predict_proba(self, waveform, local_condition_batch=None, gc_embedding=None, name='wavenet'):
...
#             gc_embedding = self._embed_gc(global_condition)
    ...
    def predict_proba_incremental(self, waveform, local_condition_batch=None, gc_embedding=None,
                                  name='wavenet'):
...
#             gc_embedding = self._embed_gc(global_condition)
            raw_output = self._create_generator(encoded, local_condition_batch, gc_embedding)
            out = tf.reshape(raw_output, [-1, self.quantization_channels])
            proba = tf.cast(
                tf.nn.softmax(tf.cast(out, tf.float64)), tf.float32)
            last = tf.slice(
